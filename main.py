...
def _to_fancy(text: str) -> str:
    """Convert ASCII letters to Unicode Mathematical Bold Script for the turn indicator."""
    out = []
...
def hp_bar(cur, mx, length=12, bare=False):
    ratio = max(0, min(1, cur/max(mx,1)))
    filled = int(ratio*length)
    bar = "█"*filled + "░"*(length-filled)
    if bare:
        return bar  # just the characters — caller adds prefix + numbers
    pct = int(ratio*100)
...
def apply_effects_to_state(effects, session, who):
    ...
    for e in effects:
        if e in ("stun","burn","poison","bleed","scorch"):
            ...
            session[f"{who}_dodge_next"]=True; notes.append("<tg-emoji emoji-id='5812074410667938253'>⚡</tg-emoji> Dodge next!")
        elif e=="multi_hit":
...
def fmt_pvp_text(state, log=""):
    ...
    # ── Opponent (top, red) ─────────────────────────────────────
    txt += f"<tg-emoji emoji-id='5798530836890390483'>🔴</tg-emoji> {otrans}<b>{hesc(oc)}</b> [{hesc(onat)} {nat_icon(onat)}]\n"
    txt += f"Lv.{olv} | HP : {ohp}/{omx}{oss}{odt}\n"
    txt += f"<tg-emoji emoji-id='5798530836890390483'>🔴</tg-emoji>[{hp_bar(ohp,omx,bare=True)}] {ohp}/{omx}\n\n"

    # ── Current player (bottom, blue) ──────────────────────────
    txt += f"<tg-emoji emoji-id='5798483635199807367'>🔵</tg-emoji> {ttrans}<b>{hesc(tc)}</b> [{hesc(tnat)} {nat_icon(tnat)}]\n"
    txt += f"Lv.{tlv} | HP : {thp}/{tmx}{tss}{tdt}\n"
    txt += f"<tg-emoji emoji-id='5798483635199807367'>🔵</tg-emoji>[{hp_bar(thp,tmx,bare=True)}] {thp}/{tmx}\n"
    ...
    if tcd:
        ...
        for m in get_char_moves(tc, tlv):
            eff = fmt_effect(m.get("effect",""))
            eff_part = f" | {eff}" if eff else ""
            txt += f"• <b>{hesc(m['name'])}</b> [{hesc(m['type'])} {nat_icon(m['type'])}]\n"
            txt += f"  Pwr:{m['power']} | Acc:{m['acc']}% | Crit:{tbase_crit}%{eff_part}\n"

    # ── Turn indicator with fancy Unicode name ─────────────────
    fancy_name = _to_fancy(tn.get(tid, tc)[:16])
    txt += f"\n<b>Turn :</b> [{fancy_name}]\n"

    # ── Transform available? ───────────────────────────────────
    if state.get("transform_avail",{}).get(tid) and not state.get("transformed",{}).get(tid):
        txt += f"<tg-emoji emoji-id='6001182426301209831'>🔥</tg-emoji> <b>TRANSFORM AVAILABLE!</b>\n"

    # ── Last turn log ──────────────────────────────────────────
    if log:
        import re as _re
        _log = _re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', log)
        txt += f"\n<b>Last:</b>\n{_log}"
...
async def safe_edit(query, text, kb, photo=None, parse_mode="HTML"):
    ...
    try:
        if photo and is_photo_msg:
            await query.edit_message_caption(
                caption=_safe_trunc_battle(text, 1020), parse_mode=parse_mode, reply_markup=Kbd(kb))
        else:
    ...
    try:
        await query.message.reply_text(
            _safe_trunc_battle(text, 1020), parse_mode=parse_mode, reply_markup=Kbd(kb))
    except Exception:
        pass
...
async def need_account(update):
    ...
    if user.get("banned"):
        ...
        return False
    # Stamp last_seen so /botstats can count online users (active ≤5 min)
    try:
        await users_col.update_one(
            {"_id": uid},
            {"$set": {"last_seen": datetime.utcnow().timestamp()}}
        )
    except Exception:
        pass
    return True
...
async def send_mychar_list(target, uid, chars, page=0, edit=False):
    ...
    if edit:
        ...
        try:
            if is_photo:
                await target.edit_message_caption(caption=_safe_trunc_battle(txt, 1020), parse_mode="HTML", reply_markup=Kbd(kb))
            else:
...
async def make_vs_photo(p_char_name: str, enemy_name: str, enemy_url=None):
    ...
    try:
        ...
        e_url = enemy_url or PHOTO_CACHE.get(enemy_name)
...
async def cb_mychar_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if parts[0] == "ml":
        ...
        batch = chars[start:start+MYCHAR_PER_PAGE]
        if not batch:
            await q.answer("No characters on this page!", show_alert=True); return
        kb = []
        for c in batch:
            cd = get_cdata(c["char_name"])
            rar = RAR.get(cd["rarity"], "⚪") if cd else "⚪"
            kb.append([Btn(f"{rar} {c['char_name']} Lv.{c['level']}",
                           callback_data=f"mc:{c['id']}:view")])
        kb.append([Btn("↩ Back to List", callback_data=f"ml:{uid}:{page}")])
        await q.answer("Tap a character to view their card!", show_alert=False)
        try:
            await q.edit_message_reply_markup(reply_markup=Kbd(kb))
        except Exception:
            pass
        return
...
async def cmd_explore(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    session = {
        ...
        "team_defeated": set(),  # tracks char IDs that have been KO'd
        # Weapon data
    ...
    if battle_photo:
        try:
            await update.message.reply_photo(
                photo=battle_photo, caption=_safe_trunc_battle(encounter_txt, 1020),
                parse_mode="HTML", reply_markup=Kbd(kb))
...
async def cb_explore(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action=="fight":
        ...
        if vs_photo:
            try:
                ...
                session["is_photo"] = True  # VS banner is now the active message
                return
            except Exception:
                pass
    ...
    if action=="sw":
        ...
        if sw_photo:
            try:
                await q.message.reply_photo(photo=sw_photo, caption=_safe_trunc_battle(txt, 1020),
                    parse_mode="HTML", reply_markup=Kbd(kb))
                session["is_photo"] = True
                return
            except: pass
    ...
    if action=="swlist":
        ...
        _defeated  = session.get("team_defeated", set())
        others     = [team_map[cid] for cid in team_order
                      if cid != cur_id and team_map.get(cid) and cid not in _defeated]
        if not others:
    ...
    if session.get("p_status")=="stun":
        ...
        res = {"hit": False, "effects": []}
    elif session.get("e_dodge_next"):
        # Enemy prepared to dodge — cancel player attack
        session["e_dodge_next"] = False
        log += f"⚔️ <b>{hesc(move['name'])}</b> — <b>{hesc(session['enemy']['name'])} DODGED!</b>\n"
        res = {"hit": False, "effects": []}
    else:
        ...
        if res["hit"]:
            ...
            # Handle self_dmg recoil (e.g. "self_dmg:30" = 30% of damage dealt)
            for _eff in res["effects"]:
                if isinstance(_eff, str) and _eff.startswith("self_dmg:"):
                    try:
                        _pct = int(_eff.split(":")[1])
                        _sd = max(1, int(res["dmg"] * _pct / 100))
                        session["p_hp"] = max(0, session["p_hp"] - _sd)
                        log += f"  💔 Recoil: <b>-{_sd} HP</b>!\n"
                    except Exception:
                        pass
        else:
        ...
        # ── ATTACK PHOTO ──────────────────────────────────────────────────────
        if res["hit"]:
            j_img = (PHOTO_CACHE.get(session["p_char"])
                     or MOVE_IMAGE_CACHE.get(move["name"])
                     or JUTSU_CACHE.get(move["name"]))
            if j_img:
                try:
                    crit_tag = "💥 <b>CRITICAL!</b> " if res.get("crit") else ""
                    cap = (f"{crit_tag}<b>{hesc(session['p_char'])}</b> uses "
                           f"<b>{hesc(move['name'])}</b>!\n"
                           f"  {res['eff_txt']} <b>-{res['dmg']} HP</b>").strip()
                    if q.message and q.message.photo:
                        await q.message.edit_media(
                            media=InputMediaPhoto(media=j_img,
                                                  caption=_safe_trunc_battle(cap, 1020),
                                                  parse_mode="HTML"))
                    else:
                        await q.message.reply_photo(photo=j_img,
                                                    caption=_safe_trunc_battle(cap, 1020),
                                                    parse_mode="HTML")
                except Exception:
                    pass
    ...
    if session["e_hp"]<=0:
        wild=session["enemy"]
        # NC coins: 4-6 per battle, no daily cap — always awarded on kill
        nc_r    = random.randint(4, 6)
    ...
    if session.get("e_status")=="stun":
        ...
        # Player dodge_next: if player prepared a dodge, skip enemy attack entirely
        if session.get("p_dodge_next"):
            session["p_dodge_next"] = False
            log+=f"  <tg-emoji emoji-id='5812074410667938253'>⚡</tg-emoji> You DODGED <b>{hesc(em['name'])}</b>!\n"
        else:
    ...
    if session["p_hp"]<=0:
        if not session.get("p_revived") and KG.get(session["p_kg"],{}).get("revive"):
            ...
            # Mark this char as defeated so swlist won't show them again
            session.setdefault("team_defeated", set()).add(session["p_char_id"])
...
async def cb_pvp_move(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Detect photo mode before any edits so it's available throughout this handler
    is_photo_msg = bool(q.message and q.message.photo)
    ...
    if state["status"][tid]=="stun":
        ...
        if state["dodge_next"].get(oid):
            ...
            # ── ATTACK PHOTO: edit in-place only — never send a new message ──────
            j_img = (PHOTO_CACHE.get(tc)
                     or MOVE_IMAGE_CACHE.get(move["name"])
                     or JUTSU_CACHE.get(move["name"]))
            if j_img and is_photo_msg:
                try:
                    ...
                    await q.message.edit_media(
                        ...
                        reply_markup=Kbd(move_btns_pvp(
                            get_char_moves(state["cur"][oid], state["lv"].get(oid,1))
                            if get_cdata(state["cur"][oid]) else [], c_uid, t_uid,
                            state["transform_avail"].get(oid, False))))
                    state["is_photo"] = True
                    is_photo_msg = True
                except Exception:
                    pass
    ...
    def check_ko(vid, ally_id):
        ...
        if state["chars"][vid][vc]["hp"]<=0:
            ...
            if nc2:
                ...
                return f"  <tg-emoji emoji-id='5426953124256424318'>💀</tg-emoji> <b>{hesc(vc)}</b> fainted! → <b>{hesc(nc2)}</b> enters!\n",False
            return "",True
        return "",False

    # Helper: best available photo for a team member ─────────────────────────
    def _team_photo(uid_key):
        """Return the first working photo URL/BytesIO for any char in uid's team."""
        for cn in state["chars"].get(uid_key, {}):
            p = PHOTO_CACHE.get(cn)
            if p:
                return p
        return None
    ...
    if all_dead:
        ...
        win_char=state["cur"][tid]
        # Use winning char photo, fall back to any team char photo
        win_photo = PHOTO_CACHE.get(win_char) or _team_photo(tid)
        pvp_hp_left = state["hp"].get(tid, 0)
        pvp_mhp = state["mhp"].get(tid, 1)
        pvp_hp_pct = int((pvp_hp_left / pvp_mhp) * 100) if pvp_mhp else 0
        if pvp_hp_pct >= 80:   pvp_hp_txt = "Dominated the duel!"
        elif pvp_hp_pct >= 50: pvp_hp_txt = "Solid performance."
        elif pvp_hp_pct >= 20: pvp_hp_txt = "Hard-fought win — barely standing!"
        else:                  pvp_hp_txt = "Survived by a miracle!"
        pvp_loser_name = lu.get("nickname") or lu["name"]
        win_txt=(
                 f"{pe('trophy')} <b>PvP VICTORY!</b>\n"
                 f"━━━━━━━━━━━━━━━━━━━━\n"
                 f"{pe('sword')} <b>{hesc(wu.get('nickname') or wu['name'])}</b> defeated <b>{hesc(pvp_loser_name)}</b>!\n"
                 f"{pe('star')} Winning character: <b>{hesc(win_char)}</b>\n"
                 ...
                 f"━━━━━━━━━━━━━━━━━━━━\n"
                 f"{log}")
        composite = await make_pvp_win_image(ctx.bot, tid, win_photo, state)
        safe_win = _safe_trunc_battle(win_txt, 1020)
        is_ph = bool(q.message and q.message.photo)
        if composite:
            try:
                if is_ph:
                    await q.message.edit_media(
                        media=InputMediaPhoto(media=composite, caption=safe_win, parse_mode="HTML"))
                else:
                    await q.message.reply_photo(photo=composite, caption=safe_win, parse_mode="HTML")
                return
            except: pass
        try:
            if is_ph:
                await q.edit_message_caption(caption=safe_win, parse_mode="HTML")
            else:
                await q.edit_message_text(win_txt, parse_mode="HTML")
        except: pass
        return

    # ── If opponent's char switched, briefly show the new char's photo ────────
    if kn and state["cur"][oid] != oc:
        sw_photo = PHOTO_CACHE.get(state["cur"][oid])
        if sw_photo and is_photo_msg:
            try:
                sw_cap = f"<tg-emoji emoji-id='5426953124256424318'>💀</tg-emoji> <b>{hesc(oc)}</b> fainted!\n<tg-emoji emoji-id='5812074410667938253'>⚡</tg-emoji> <b>{hesc(state['cur'][oid])}</b> enters!"
                await q.message.edit_media(
                    media=InputMediaPhoto(media=sw_photo,
                                         caption=_safe_trunc_battle(sw_cap, 1020),
                                         parse_mode="HTML"))
            except: pass
    ...
    if self_dead:
        ...
        win_char2=state["cur"][oid]
        win_photo2 = PHOTO_CACHE.get(win_char2) or _team_photo(oid)
        win_txt2=(f"<tg-emoji emoji-id='5426976939850080222'>🏆</tg-emoji> <b>VICTORY!</b>\n"
                  f"━━━━━━━━━━━━━━━━━━━━\n"
                  f"{log}\n"
                  f"<tg-emoji emoji-id='5426976939850080222'>🥇</tg-emoji> <b>{hesc(wu.get('nickname') or wu['name'])}</b> wins!\n"
                  f"<tg-emoji emoji-id='6035338995536238141'>⚔️</tg-emoji> Winner's Char: <b>{hesc(win_char2)}</b>\n"
        ...
        composite2 = await make_pvp_win_image(ctx.bot, oid, win_photo2, state)
        safe_win2 = _safe_trunc_battle(win_txt2, 1020)
        is_ph2 = bool(q.message and q.message.photo)
        if composite2:
            try:
                if is_ph2:
                    await q.message.edit_media(
                        media=InputMediaPhoto(media=composite2, caption=safe_win2, parse_mode="HTML"))
                else:
                    await q.message.reply_photo(photo=composite2, caption=safe_win2, parse_mode="HTML")
                return
            except: pass
        try:
            if is_ph2:
                await q.edit_message_caption(caption=safe_win2, parse_mode="HTML")
            else:
                await q.edit_message_text(win_txt2, parse_mode="HTML")
        except: pass
        return

    # ── If current player's char switched, show the new char entering ─────────
    if kn2 and state["cur"][tid] != tc:
        sw_photo2 = PHOTO_CACHE.get(state["cur"][tid])
        if sw_photo2 and is_photo_msg:
            try:
                sw_cap2 = f"<tg-emoji emoji-id='5426953124256424318'>💀</tg-emoji> <b>{hesc(tc)}</b> fainted!\n<tg-emoji emoji-id='5812074410667938253'>⚡</tg-emoji> <b>{hesc(state['cur'][tid])}</b> enters!"
                await q.message.edit_media(
                    media=InputMediaPhoto(media=sw_photo2,
                                         caption=_safe_trunc_battle(sw_cap2, 1020),
                                         parse_mode="HTML"))
            except: pass
    ...
    is_photo_now = bool(q.message and q.message.photo)
    try:
        if is_photo_now:
            await q.edit_message_caption(caption=_safe_trunc_battle(txt, 1020),
                                         parse_mode="HTML", reply_markup=Kbd(kb))
        else:
            await q.edit_message_text(txt, parse_mode="HTML", reply_markup=Kbd(kb))
    except Exception:
...
# ─── WIN IMAGE HELPER ───────────────────────────────────────────
async def make_pvp_win_image(bot, winner_uid: str, char_photo_url, state=None):
    """Composite winner's Telegram DP (left) + winning char / team photo (right)."""
    if not PIL_AVAILABLE:
        return None
    try:
        W, H = 460, 240
        HALF_W = 230
        base = Image.new("RGB", (W, H), (15, 15, 30))
        draw = ImageDraw.Draw(base)

        async def _load_url(url) -> "Image.Image | None":
            if not url:
                return None
            try:
                import httpx
        ...
        def _fit(img, w, h):
            ...
            bg = Image.new("RGB", (w, h), (25, 25, 45))
            ox = (w - img.width) // 2
            oy = (h - img.height) // 2
            bg.paste(img, (ox, oy))
            return bg

        # ── Left: winner's Telegram profile photo ──────────────────────────────
        try:
            ...
            if photos and photos.photos:
                ...
                base.paste(_fit(left_img, HALF_W, H), (0, 0))
        except Exception:
            pass

        # ── Right: winning char, then fall back through entire winner team ──────
        right_img = None
        # Try the primary win char photo first
        if char_photo_url:
            right_img = await _load_url(char_photo_url)
        # If that failed and we have state, try every team char in order
        if right_img is None and state:
            for cn in state.get("chars", {}).get(winner_uid, {}):
                p = PHOTO_CACHE.get(cn)
                if p:
                    right_img = await _load_url(p)
                    if right_img:
                        break
        if right_img:
            base.paste(_fit(right_img, HALF_W, H), (HALF_W, 0))
        else:
            # Solid colour placeholder so there's never a plain black gap
            placeholder = Image.new("RGB", (HALF_W, H), (40, 30, 60))
            draw_ph = ImageDraw.Draw(placeholder)
            draw_ph.text((HALF_W // 2 - 20, H // 2 - 10), "🏆 WIN", fill=(180, 150, 255))
            base.paste(placeholder, (HALF_W, 0))

        # ── Gold divider + trophy ───────────────────────────────────────────────
        draw.line([(HALF_W - 1, 0), (HALF_W - 1, H)], fill=(255, 215, 0), width=3)
        try:
            fnt = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 22)
        except Exception:
            fnt = ImageFont.load_default()
        lbl = "VICTORY!"
        draw.text((HALF_W - 38, 6), lbl, fill=(255, 215, 0), font=fnt)

        out = io.BytesIO()
        base.save(out, format="JPEG", quality=85)
        out.seek(0)
        out.name = "pvp_win.jpg"
        return out
    except Exception:
        return None
...
async def cmd_travel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.message.reply_photo(
            photo=VILLAGE_BANNER, caption=_safe_trunc_battle(txt, 1020),
...
async def cb_travel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await q.edit_message_caption(caption=_safe_trunc_battle(cap, 1020), parse_mode="HTML",
                                     reply_markup=Kbd(kb))
    except:
        try: await q.edit_message_text(_safe_trunc_battle(cap, 1020), parse_mode="HTML",
                                       reply_markup=Kbd(kb))
        except: pass
...
async def cb_travel_explore(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if battle_photo:
        try:
            await q.edit_message_caption(caption=_safe_trunc_battle(txt, 1020), parse_mode="HTML",
                                         reply_markup=Kbd(kb))
            session["is_photo"] = True
            return
        except: pass
    try:
        await q.edit_message_text(_safe_trunc_battle(txt, 4096), parse_mode="HTML", reply_markup=Kbd(kb))
    except: pass
...
async def cb_travel_back(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try: await q.edit_message_text(_safe_trunc_battle(txt, 1020), parse_mode="HTML",
                                       reply_markup=Kbd(rows))
        except: pass
...
async def cmd_botstats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Measure ping — time from user's message to bot processing it
    import time as _time
    msg_ts = update.message.date.timestamp()
    now_ts = _time.time()
    ping_ms = int((now_ts - msg_ts) * 1000)

    # Count online users (last_seen within the past 5 minutes)
    online_cutoff = now_ts - 300
    online_count = await users_col.count_documents({"last_seen": {"$gte": online_cutoff}})
    ...
    agg = await users_col.aggregate([
        {"$group": {"_id": None, "total_wins": {"$sum": "$wins"}}}
    ]).to_list(1)
    total_wins = agg[0]["total_wins"] if agg else 0

    # Ping quality label
    if ping_ms < 300:
        ping_label = "🟢 Excellent"
    elif ping_ms < 800:
        ping_label = "🟡 Good"
    elif ping_ms < 1500:
        ping_label = "🟠 Fair"
    ...
    await update.message.reply_text(
        ...
        f"🟢 Online Now <i>(last 5 min)</i>: <b>{online_count:,}</b>\n"
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📡 Ping: <b>{ping_ms} ms</b> — {ping_label}",
        parse_mode="HTML")
