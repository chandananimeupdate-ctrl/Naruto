...
_EFF_MAP = {
    "burn":"<tg-emoji emoji-id='6001182426301209831'>🔥</tg-emoji>Burn","stun":"<tg-emoji emoji-id='5812074410667938253'>⚡</tg-emoji>Stun","poison":"☠️Poison","bleed":"<tg-emoji emoji-id='5960999309280284625'>🩸</tg-emoji>Bleed",
    "aoe":"<tg-emoji emoji-id='5998841304052669228'>💥</tg-emoji>AoE","def_up":"<tg-emoji emoji-id='6039727447090403055'>🛡️</tg-emoji>+DEF","spd_down":"🐢-SPD","multi_hit":"✦Multi",
    "pierce":"<tg-emoji emoji-id='6035338995536238141'><tg-emoji emoji-id='6035338995536238141'>🗡</tg-emoji>️</tg-emoji>Pierce","crit":"💢+Crit","def_break":"Break DEF",
    "soul_rip":"<tg-emoji emoji-id='5426953124256424318'>💀</tg-emoji>SoulRip","skip_turn":"⏭️Skip","unavoidable":"Sure-Hit",
    "dodge_next":"<tg-emoji emoji-id='5426957509418031897'>🌀</tg-emoji>Dodge","reflect_dmg":"🪞Reflect","seal":"🔒Seal",
    "chain":"<tg-emoji emoji-id='5958595510573994440'>⛓</tg-emoji>️Chain","acc_down":"<tg-emoji emoji-id='6035051267087143217'><tg-emoji emoji-id='6035051267087143217'>👁</tg-emoji>️</tg-emoji>-Acc","gravity_pull":"🌑Gravity",
}
...
async def cb_mychar_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if parts[0] == "ml":
        ...
        if not chars: return
        start = page * MYCHAR_PER_PAGE
        # Show first char on this page in detail
        await send_char_card(q.message, ctx, uid, start, chars, edit=False)
...
async def cb_explore(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action=="fight":
        ...
        await safe_edit(q, txt[:1020], kb, enemy_photo)
        return
    ...
    if session.get("p_status")=="stun":
        ...
        # ── ATTACK PHOTO: send as separate reply so battle buttons never break ──
        j_img = (PHOTO_CACHE.get(session["p_char"])
                 or MOVE_IMAGE_CACHE.get(move["name"])
                 or JUTSU_CACHE.get(move["name"]))
        if j_img:
            try:
                crit_tag = "💥 CRITICAL! " if res.get("crit") else ""
                dmg_tag  = f"-{res['dmg']} HP" if res["hit"] else "MISSED"
                cap = (f"{crit_tag}<b>{hesc(session['p_char'])}</b> uses "
                       f"<b>{hesc(move['name'])}</b>! {dmg_tag}").strip()
                await q.message.reply_photo(photo=j_img, caption=cap[:1020], parse_mode="HTML")
            except Exception:
                pass
